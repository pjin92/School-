...
<?
	include "../common.php";
	$query="select name59 from opt where no59=$no1;";
	$result=mysql_query($query);
	if(!$result) exit("쿼리에러1");
...
<table width="450" border="0" cellspacing="0" cellpadding="0">
	<tr height="50">
		<td align="left"  width="300" valign="bottom">&nbsp 옵션명 : <font color="#0457A2"><b><?=$row[name59]?></b></font></td>
...
<table width="500" border="1" cellspacing="0" bordercolordark="white" bordercolorlight="black">
	...
	<?
	  $query="select * from opts where opt_no59=$no1 order by no59";
	  $result=mysql_query($query);
	  if(!$result) exit("쿼리에러");
	  $count=mysql_num_rows($result);
	  ...
	  for($i=0;$i<$page_last;$i++)
	  {
		...
		echo("<tr bgcolor='#F2F2F2' height='23'>
				...
				<td align='center'>
				<a href='opts_edit.php?no1=$row[opt_no59]&no2=$row[no59]&page=$page&sel1=$sel1&text1=$text1'>수정</a>/
				<a href='opts_delete.php?no1=$row[opt_no59]&no2=$row[no59]&page=$page&sel1=$sel1&text1=$text1' onclick='javascript:return confirm(\"삭제할까요 ?\");'>삭제</a>
			</td>
			  </tr>");
	 }
?>
...
<?
	...
	for($i=$page_s+1;$i<=$page_e;$i++)
	{
		if($page==$i)
			...
			echo("<a href='opts.php?no1=$no1&page=$i&text1=$text1'>[$i]</a>&nbsp");
	}
	if($block<$blocks)
	{
		$tmp=$page_e+1;
		echo("&nbsp<a href='opts.php?no1=$no1&page=$tmp&text1=$text1'>
