...
<?
	if($cookie_no)
	$query="select * from jumun where member_no39=$cookie_no order by no39 desc;";
	else
	$query="select * from jumun where o_name39='$name' and o_email39='$o_email' order by no39 desc;";
	...
	for($i=0;$i<$page_last;$i++)
	{
		...
		echo("<tr>
					<td height='30' align='center'><font color='#686868'>$row[jumunday39]</font></td>
					<td align='center'>
						<a href='jumun_info.php?no=$row[no39]&page=1'><font color='#0066CC'>$row[no39]</font></a>
					</td>
					<td><font color='#686868'>$row[product_names39]</font></td>
