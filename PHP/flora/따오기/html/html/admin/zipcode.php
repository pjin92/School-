...
<?
	...
	for($i=0;$i<$count;$i++)
	{
		...
		$zip2=trim(substr($row[zip39],4,3));
		$zip3="[".$zip1."-".$zip2."]"." ".$row[juso1]." ".$row[juso2]. " ". $row[juso3]." ".$row[juso4];
		$zip4=$zip1."^^".$zip2."^^".$row[juso1]." ".$row[juso2]. " ". $row[juso3];

		echo("<option value='$zip4'>$zip3</option>");
	}
?>
			</select>
		</td>
