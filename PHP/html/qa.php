...
<!-------------------------------------------------------------------------------------------->	
	...
	{
		if ($sel1==0)          // 제목
			$query="select * from qa where title68 like '%$text1%'   order by pos1 desc, pos2;";
		else if ($sel1==1)    // 내용
			$query="select * from qa where content68 like '%$text1%'   order by pos1 desc, pos2;";
		else                        // 작성자
			$query="select * from qa where name68 like '%$text1%'   order by pos1 desc, pos2;";
	}
	...
	for($i=0;$i<$page_last;$i++)
	{
		...
		echo("<tr><td height='1' bgcolor='#DEDCDD'colspan='5'></td></tr>
				<tr height='25' bgcolor='#FFFFFF'>	
					<td width='90' align='center'><font color='#686868'>$row[no68]</font></td>
					<td>");
		$n=strlen($row[pos2]);     // 문자열길이
		if ($n==1)   // 정상 글인 경우
			echo("<a href='qa_read.php?no=$row[no68]&count=$row[count68]&page=$page&sel1=$sel1&text1=$text1'>&nbsp;<font color='#4186C7'>$title68");
		else             // 답변글인 경우
		{
		for($j=0;  $j<$n-2;  $j++) echo("&nbsp;&nbsp;");
		echo("<img src='images/i_re.gif' border='0' align='absmiddle'>&nbsp;<a href='qa_read.php?no=$row[no68]&count=$row[count68]&page=$page&sel1=$sel1&text1=$text1'>&nbsp;<font color='#4186C7'>$title68");
		}
		echo("</font></a></td>	
					<td width='90' align='center'><font color='#686868'>$row[name68]</font></td>	
					<td width='90' align='center'><font color='#686868'>$row[writeday68]</font></td>	
					<td width='90' align='center'><font color='#686868'>$row[count68]</font></td>
				</tr>");
	}
