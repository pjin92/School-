...
<?
  ...
  $query="select * from opts where no68=$no1;";
...
<?
  $query="select * from opt where no68=$no1;";
  $result=mysql_query($query);
  if(!$result) exit("쿼리에러");
  $row=mysql_fetch_array($result);
?>
<input type="hidden" name="no1" value="<?=$row[no68]?>">
<?
  $query="select * from opts where no68=$no2;";
  $result=mysql_query($query);
  if(!$result) exit("쿼리에러");
  $row=mysql_fetch_array($result);
?>
<input type="hidden" name="no2" value="<?=$row[no68]?>">
...
<table width="500" border="1" cellspacing="0" bordercolordark="white" bordercolorlight="black">
	<tr> 
		...
		<td width="400" height="20"  bgcolor="#F2F2F2"><?=$row[no68]?></td>
	</tr>
	<tr> 
		<td width="100" height="20" bgcolor="#CCCCCC" align="center">
