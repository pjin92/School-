...
<?
	include "../common.php";
	$query="select * from jumun order by no68;";
...
<?
	...
	if (!$text1) $text1=""; 
	$k=0;
	$s[$k] = "date_format(jumunday68,'%Y-%m-%d')"." between '".$startdate."' and '".$enddate."'";$k++;
	if ($sel1!=0){ $s[$k] = "state68=" . $sel1;$k++;}
	if ($text1)
	{
    if ($sel2==0) { $s[$k] = "no68 like '%" .$text1. "%'";}
    elseif ($sel2==1) { $s[$k] = "o_name68 like '%" .$text1. "%'";}
	elseif ($sel2==2) { $s[$k] = "product_names68 like '%" .$text1. "%'";}
	}
	if($s>0)
	{
    $tmp=implode(" and ", $s); 
	...
	$query="select * from jumun ".$tmp." order by no68;";
	$result=mysql_query($query);
	$count=mysql_num_rows($result);
	if(!$page) $page=1;
	...
	for($i=0;$i<$page_last;$i++)
	{
		...
		if ($row1[pay_method68]==0)  
			$pay_method="카드";  
		else   
			$pay_method="현금";		
		$total_cash = number_format($row1[total_cash68]);
		$color="black";
		if ($row1[state68]==5)  $color="blue";  // 주문완료 
		if ($row1[state68]==6)  $color="red";   // 주문취소

		echo("<tr bgcolor='#F2F2F2' height='23'>	
				<td width='70' align='center'><a href='jumun_info.php?no=$row1[no68]'>$row1[no68]</td>	
				<td width='70' align='center'>$row1[jumunday68]&nbsp</td>	
				<td width='250' align='left'>&nbsp $row1[product_names68]</td>	
				<td width='40' align='center'>$row1[product_nums68]</td>	
				<td width='70' align='right'>$total_cash &nbsp</td>	
				<td width='65' align='center'>$row1[o_name68]</td>
    ...
    if ($j==$row1[state68])
		...
		echo("	</select>
				<a href='javascript:go_update(\"$row1[no68]\",$i);'><img src='images/b_edit1.gif' border='0'></a>
				</td>
				<td width='50' align='center' valign='bottom'>
				<a href='jumun_delete.php?no=$row1[no68]&day1_y=$day1_y&day1_m=$day1_m&day1_d=$day1_d&day2_y=$day2_y&day2_m=$day2_m&day2_d=$day2_d&sel1=$sel1&sel2=$sel2&text1=$text1'onclick='javascript:return confirm(\"삭제할까요 ?\");'><img src='images/b_delete1.gif' border='0'></a>
				</td>
			 </tr>");
	}
?>
