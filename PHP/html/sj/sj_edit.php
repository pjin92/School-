...
<?
  $query="select * from sj where no68=$no;";
  $result=mysql_query($query);
  if(!$result) exit("쿼리에러");
$row=mysql_fetch_array($result);
$avg=sprintf("%6.1f",$row[avg68]);
?>
...
<table width="300" border="1" cellpadding="1" cellspacing="0" bgcolor="lightyellow">
  <tr>
    ...
    <td width="200">
      <input type="text" name="name" size="20" value="<?=$row[name68]?>">
    </td>
  </tr>
  <tr>
    <td width="100" align="center" bgcolor="lightblue">국어</td>
    <td width="200">
      <input type="text" name="kor" size="6" value="<?=$row[kor68]?>" onChange="javascript:cal_jumsu();">
    </td>
  </tr>
  <tr>
    <td width="100" align="center" bgcolor="lightblue">영어</td>
    <td width="200">
      <input type="text" name="eng" size="6" value="<?=$row[eng68]?>" onChange="javascript:cal_jumsu();">
    </td>
  </tr>
  <tr>
    <td width="100" align="center" bgcolor="lightblue">수학</td>
    <td width="200">
      <input type="text" name="mat" size="6" value="<?=$row[mat68]?>" onChange="javascript:cal_jumsu();">
    </td>
  </tr>
  <tr>
    <td width="100" align="center" bgcolor="lightblue">총점</td>
    <td width="200">
      <input type="text" name="hap" size="6" value="<?=$row[hap68]?>" readonly style="border:0;background-color:#ffffe0">
    </td>
  </tr>
  <tr>
